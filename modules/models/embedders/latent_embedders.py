...
class VAEGAN(VeryBasicModel):
    ...
    def encode_timestep(self, timestep):
        if self.vqvae.time_embedder is None:
            ...
            return self.vqvae.time_embedder(timestep)
    
    def encode(self, x, emb=None):
        return self.vqvae.encode(x, emb=emb) 
            
    def decode(self, z, emb=None):
        return self.vqvae.decode(z, emb=emb)

    def forward(self, x, timestep=None):
        return self.vqvae.forward(x, timestep=timestep)
    
    
    def vae_img_loss(self, pred, target, dec_out_layer, step, discriminator, depth=0, split='train'):
         # ------ VQVAE -------
        rec_loss = self.vqvae.rec_loss(pred, [], target)
        ...
        if (step > self.start_gan_train_step) and (depth < 2):
            ...
            lambda_weight = self.compute_lambda(rec_loss, gan_loss, dec_out_layer) if split == 'train' else torch.tensor(1.0)
            gan_loss = gan_loss * lambda_weight
    ...
    def gan_img_loss(self, pred, target, step, discriminators, depth):
        ...
        with torch.no_grad():
            self.log(f"train/d_loss_{depth}", loss, on_step=True, on_epoch=True) 
    ...
    def _step(self, batch, batch_idx, split, step):
        ...
        # Horizontal/Top Layer 
        rec_loss = self.vae_img_loss(pred, target, self.vqvae.outc.conv, step, self.discriminator, 0, split=split)

        # Vertical/Deep Layer 
        for i, pred_i in enumerate(pred_vertical): 
            target_i = F.interpolate(target, size=pred_i.shape[2:], mode=interpolation_mode, align_corners=None)  
            rec_loss += self.vae_img_loss(pred_i, target_i, self.vqvae.outc_ver[i].conv, step, self.discriminator, i + 1, split=split)

        # compute loss
        ae_loss = rec_loss + self.embedding_loss_weight * emb_loss

        with torch.no_grad():
            logging_dict['rec_loss'] = rec_loss
            logging_dict['kl_loss'] = emb_loss
        ...
        # Horizontal/Top Layer 
        d_loss = self.gan_img_loss(pred, target, step, self.discriminator, 0)

        # Vertical/Deep Layer 
        for i, pred_i in enumerate(pred_vertical):  
            target_i = F.interpolate(target, size=pred_i.shape[2:], mode=interpolation_mode, align_corners=None)  
            d_loss += self.gan_img_loss(pred_i, target_i, step, self.discriminator, i + 1)
            
        with torch.no_grad():
            logging_dict['d_loss'] = d_loss
        ...
        # ----------------- Log Scalars ----------------------
        for metric_name, metric_val in logging_dict.items():
            self.log(f"{split}/{metric_name}", metric_val, batch_size=x.shape[0], on_step=True, on_epoch=True)     
